...
- 全屏 Dialog/Modal 可以挂载到 `document.body` 下的隔离宿主，宿主必须携带 `.zt-tiptap-theme` 并同步编辑器主题变量，避免被父级 `overflow` 或 `transform` 裁切。
- Toolbar/BubbleMenu 全局类菜单等锚点浮层必须挂在编辑器容器内的 `.zt-tiptap-portal`（通过 `portalContainer`），不要挂到 `document.body`。
